...
public class MyBigNumber 
{
	...
	{	
		...
		for (int i = 0; i < len1; i++) // kiem tra ki tu 
		{
		    if ( Character.isLetter( s1.charAt(i) ) ) 
		     {
		    	throw new NumberFormatException("vi tri: " + (i + 1) + " chuoi " + s1+" co ki tu ");
		     }
		}
		...
		for (int i = 0; i < len2; i++) 
		{
		        ...
		        {
		        	throw new NumberFormatException("vi tri: " + (i + 1) + " chuoi " + s2+" co ki tu");
		        }
		}
		
		Pattern pattern1 = Pattern.compile("[!@#$%&*()_+=|<>?{}\\\\[\\\\]~-]"); // kiem tra ki tru dac biet
		Matcher matcher1 = pattern1.matcher( s1 );
		Matcher matcher2 = pattern1.matcher( s2 );
		if (matcher1.find()) 
		{
            throw new NumberFormatException("Vi tri: " + (matcher1.start() + 1) + " chuoi " + s1
            + " co ki tu dac biet");
		}
			
		if (matcher2.find()) 
		{
            throw new NumberFormatException("Vi tri:" + (matcher2.start() + 1) + " Vi tri: " + s2
            + "co ki tu dac biet");
		}
